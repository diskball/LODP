...
function cfxOwnedZones.verifyNeutralAirports()
	-- Ensures that neutral zones always have their airports deactivated for both coalitions
	-- This is called periodically to maintain consistency and prevent slot access during neutral state
	for idx, theZone in pairs(cfxOwnedZones.zones) do
		if (theZone.owner == 0 or theZone.owner == 3) and theZone.controlsAirport and theZone.controlsAirport ~= "none" then
			-- This zone is neutral or contested - verify airport is deactivated
			local airportName = theZone.controlsAirport
			local airport = AIRBASE:FindByName(airportName)
			if airport then
				-- Double-check that both coalitions are deactivated
				...
				if cfxOwnedZones.verbose then
					trigger.action.outText("+++owdZ: Verified airport '" .. airportName .. "' deactivated for neutral zone '" .. theZone.name .. "'", 30)
				end
			end
		end
	end
...
function cfxOwnedZones.zoneConquered(aZone, theSide, formerOwner) -- 0 = neutral 1 = RED 2 = BLUE 3 = CONTESTED
	...
	if aZone.controlsAirport and aZone.controlsAirport ~= "none" then
		...
		local status, err = pcall(function()
			local airport = AIRBASE:FindByName(airportName)
			if airport then
				if theSide == 1 then -- RED owns zone
					trigger.action.airportDeactivate(airport:GetID(), coalition.side.BLUE)
					trigger.action.airportActivate(airport:GetID(), coalition.side.RED)
					if MESSAGE then MESSAGE:New("Airport " .. airportName .. " is now controlled by REDFOR.", 10):ToAll() end
				elseif theSide == 2 then -- BLUE owns zone
					trigger.action.airportDeactivate(airport:GetID(), coalition.side.RED)
					trigger.action.airportActivate(airport:GetID(), coalition.side.BLUE)
					if MESSAGE then MESSAGE:New("Airport " .. airportName .. " is now controlled by BLUEFOR.", 10):ToAll() end
				elseif theSide == 0 or theSide == 3 then -- Neutral or Contested
					trigger.action.airportDeactivate(airport:GetID(), coalition.side.RED)
					trigger.action.airportDeactivate(airport:GetID(), coalition.side.BLUE)
					if MESSAGE then MESSAGE:New("Airport " .. airportName .. " is now neutral/contested. Slots unavailable.", 10):ToAll() end
				end
			else
				if cfxOwnedZones.verbose then
					trigger.action.outText("+++owdZ: WARNING: Configured airport '" .. airportName .. "' for zone '" .. aZone.name .. "' not found.", 30)
				end
			end
		end)
		if not status and cfxOwnedZones.verbose then
			trigger.action.outText("+++owdZ: ERROR handling airport for zone '" .. aZone.name .. "': " .. tostring(err), 30)
		end
	end
...
function cfxOwnedZones.readConfigZone(theZone)
	...
	cfxOwnedZones.numCap = theZone:getNumberFromZoneProperty("numCap", 3) -- minimal number of units required to cap zone (increased to 3 to prevent accidental single-unit captures)
	cfxOwnedZones.numKeep = theZone:getNumberFromZoneProperty("numKeep", 1) -- minimum units needed to keep an already-owned zone
...
function cfxOwnedZones.init()
	-- check libs
	trigger.action.outText("+++owdZ: Starting initialization...", 30)
	
	if not dcsCommon.libCheck("cfx Owned Zones", 
		cfxOwnedZones.requiredLibs) then
		trigger.action.outText("+++owdZ: FATAL - libCheck failed", 30)
		return false 
	end
	trigger.action.outText("+++owdZ: libCheck passed", 30)

	-- read my config zone
	local theZone = cfxZones.getZoneByName("ownedZonesConfig")
	if not theZone then
		trigger.action.outText("+++owdZ: WARNING - ownedZonesConfig zone not found, using defaults", 30)
	end
	trigger.action.outText("+++owdZ: Reading config zone...", 30)
	cfxOwnedZones.readConfigZone(theZone)
	trigger.action.outText("+++owdZ: Config zone processed", 30)

	
	-- collect all owned zones by their 'owner' property 
	-- start the process
	trigger.action.outText("+++owdZ: Collecting zones with 'owner' property...", 30)
	local pZones = cfxZones.zonesWithProperty("owner")
	trigger.action.outText("+++owdZ: Found " .. #pZones .. " zones with 'owner' property", 30)
...
-- Also wrap in pcall to catch any hidden errors
local status, err = pcall(function()
	if cfxOwnedZones and cfxOwnedZones.version then
		-- init was called, check if it completed
		if not cfxOwnedZones.initialized then
			trigger.action.outText("+++owdZ: WARNING - init was called but did not complete. Check zone configuration.", 30)
		end
	end
end)

if not status then
	trigger.action.outText("cf/x Owned Zones FATAL ERROR: " .. tostring(err), 30)
end
...
--[[--
	...
	- BUGFIX v2.5.3: Fixed numKeep logic that was removed during simplification. Zones
	  ...
	  to capture them.
	- ENHANCEMENT: Increased default numCap from 1 to 3 to prevent accidental single-unit
	  zone captures. A zone now requires at least 3 units to be captured (can be overridden
	  per-zone via numCap property).
	- ENHANCEMENT: Added comprehensive verbose logging at every initialization step to
	  diagnose zone loading issues. Shows config zone reading, zone collection, zone
	  addition, persistence registration, and timer scheduling.
	...
	- numCap = minimum units needed to capture a zone (default: 3 to prevent accidental captures)
	...
	AIRPORT CONTROL BEHAVIOR:
	- RED/BLUE owner: Airport active for that coalition only
	- NEUTRAL (owner=0): Airport deactivated for both coalitions
	- CONTESTED (owner=3): Airport deactivated for both coalitions
	- Airport control transitions occur when zone ownership changes
