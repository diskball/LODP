...
function cfxPlayerScore.createNewPlayerScore(playerName)
    ...
    thePlayerScore.featQueue = {} -- when using deferred
    thePlayerScore.totalFeats = 0
    thePlayerScore.sortieBankAccu = 0 -- bank funds earned this sortie, paid out on landing
    return thePlayerScore
end
...
function cfxPlayerScore.wipeScore(playerName) 
    ...
    thePlayerScore.sortieBankAccu = 0
...
function cfxPlayerScore.updateScoreForPlayerImmediate(playerName, score, reason, data)
    ...
    -- fratricide only deducts from personal score, never coalition or bank
    if reason ~= "FRAT" and pFaction > 0 then
...
function cfxPlayerScore.handlePlayerLanding(theEvent)
    ...
    -- pay out sortie bank funds on landing (non-deferred bankOnLanding mode)
    local isDeferred = cfxPlayerScore.isDeferred(playerName)
    if cfxPlayerScore.bankOnLanding and not isDeferred then
        theScore = cfxPlayerScore.getPlayerScore(playerName)
        local bankAccu = theScore.sortieBankAccu or 0
        if bankAccu > 0 and bank and bank.addFunds then
            local pFaction = dcsCommon.playerName2Coalition(playerName)
            if pFaction > 0 then
                bank.addFunds(pFaction, bankAccu)
                local uid = thePlayerUnit:getID()
                trigger.action.outTextForUnit(uid, "Landing bonus: §" .. bankAccu .. " added to coalition funds.", 20)
            end
        end
        theScore.sortieBankAccu = 0
        cfxPlayerScore.setPlayerScore(playerName, theScore)
    end
    -- see if we are using deferred scoring, else can end right now
    if not isDeferred then return end
...
function cfxPlayerScore.readConfigZone(theZone)
    ...
    cfxPlayerScore.bankOnLanding = theZone:getBoolFromZoneProperty("bankOnLanding", false) -- defer bank payout until landing
end
...
function cfxPlayerScore.start()
    ...
    -- read verbose flag early
    local theConfigZone = cfxZones.getZoneByName("playerScoreConfig")
    if theConfigZone then cfxPlayerScore.verbose = theConfigZone.verbose end

    -- load score table: cfxScoreTableData global (set by cfxScoreTable.lua) → playerScoreTable zone fallback
    if _G["cfxScoreTableData"] then
        cfxPlayerScore.typeScore = {}
        for k, v in pairs(cfxScoreTableData) do
            cfxPlayerScore.typeScore[k:upper()] = tostring(v)
        end
        trigger.action.outText("+++pScr: loaded score table from cfxScoreTableData", 30)
    else
        local theZone = cfxZones.getZoneByName("playerScoreTable")
        if theZone then
            if cfxPlayerScore.verbose then trigger.action.outText("+++pScr: loading score table from playerScoreTable zone", 30) end
            cfxPlayerScore.typeScore = theZone:getAllZoneProperties(true)
        end
    end

    -- extract wildcard entries
    ...
    for theType, theScore in pairs(cfxPlayerScore.typeScore) do
        if dcsCommon.stringEndsWith(theType, "*") then
            ...
            if cfxPlayerScore.verbose then
                trigger.action.outText("+++PScr: wildcard <" .. wcType .. "> score <" .. theScore .. "> registered", 30)
            end
        end
    end
