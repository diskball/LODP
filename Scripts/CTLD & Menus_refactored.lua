-- ==================
-- CONFIGURATION // { id = "JTAC",      menuName = "JTAC",         ctldName = "JTAC",         type = CTLD_CARGO.Enum.VEHICLE, redGroup = "RED JTAC",      blueGroup = "BLUE JTAC",      size = 1, time = 1000, mass = 0, cost = 150 },
-- ==================
...
local UNIT_CONFIG = {
    ...
    { id = "Gaz66",     menuName = "Gaz-66 Truck", ctldName = "Gaz-66 Truck", type = CTLD_CARGO.Enum.VEHICLE, redGroup = "RED GAZ66",     blueGroup = "BLUE GAZ66",     size = 1, mass = 1000, subcategory =  "Transport", cost = 100 },
}
...
local TRUCK_CONFIG = {
    { type = "Gaz-66", sling = true, crates = true, maxCrates = 2, maxTroops = 6,  length = 9, mass = 4200 },
}
    ...
    for _, unit in ipairs(UNIT_CONFIG) do
        ...
        if groupExists then
            if unit.type == CTLD_CARGO.Enum.TROOPS then
                ctldInstance:AddTroopsCargo(unit.ctldName, { groupName }, unit.type, unit.size, unit.mass, 0, unit.subcategory)
            else
                ctldInstance:AddCratesCargo(unit.ctldName, { groupName }, unit.type, unit.size, unit.mass, 0, unit.subcategory)
            end
        else
            ...
            if string.find(unit.id, "Truck") or string.find(unit.id, "939") or string.find(unit.id, "66") then
                env.info("CTLD: Warning - Truck group '" .. groupName .. "' not found. Create this group in Mission Editor to use " .. unit.menuName)
            end
        end
    end
...
my_ctld:AllowCATransport(true, redTruckers)
...
my_ctld2:AllowCATransport(true, blueTruckers)

-- Set truck capabilities
for _, truck in ipairs(TRUCK_CONFIG) do
    my_ctld:SetUnitCapabilities(truck.type, truck.sling, truck.crates, truck.maxCrates, truck.maxTroops, truck.length,
        truck.mass)
    my_ctld2:SetUnitCapabilities(truck.type, truck.sling, truck.crates, truck.maxCrates, truck.maxTroops, truck.length,
        truck.mass)
end
